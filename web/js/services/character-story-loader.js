...
// Story scenario scripts live on the shared mirror under MyAssets/docs/spots/.
// They are not indexed in resources.json, but every script path is fully
// derivable from a story's `content_path` (e.g. "yiji/100004"), so URLs are
// resolved directly against `resources.base` at load time.
const STORY_DIR = "MyAssets/docs/spots";
...
function buildBytesSuffixes(language) {
  ...
  return uniqueValues([preferredSuffix, ...fallbacks]);
}
...
export async function loadCharacterStoryContent(contentPath, language) {
  ...
  const loadPromise = (async () => {
    ...
    const base = (resources && resources.base) || "";
    const storyUrl = (suffix) => `${base}${STORY_DIR}/${normalizedPath}${suffix}`;

    // The scenario JSON and the localized `.bytes` are independent, so fetch the
    // scenario concurrently with the bytes fallback chain (no extra requests).
    const scenarioPromise = fetchJsonResource(storyUrl(".json"));

    let bytesDictionary = null;
    for (const suffix of buildBytesSuffixes(normalizedLanguage)) {
      const bytesText = await fetchText(storyUrl(`_${suffix}.bytes`));
    ...
    const hasBytes = Boolean(bytesDictionary && bytesDictionary.size > 0);

    // The source-language `_map.json` is only a fallback for the localized
    // `.bytes`; many stories ship only `.bytes`, so fetch the map lazily to
    // avoid a redundant request (and a 404/403) when bytes already resolved.
    const mapJson = hasBytes ? null : await fetchJsonResource(storyUrl("_map.json"));
    const mapDictionary = mapJson ? parseStoryMapJson(mapJson) : new Map();
    const dictionary = hasBytes ? bytesDictionary : mapDictionary;

    const scenarioJson = await scenarioPromise;
